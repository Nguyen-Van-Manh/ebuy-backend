...
export class ShippingAddressService {
  ...
  async getShippingAddressDetail(
    userId: string,
    id: Types.ObjectId,
  ): Promise<ShippingAddressGraphQL | undefined> {
    try {
  ...
  async updateShippingAddress(
    userId: string,
    addressId: string,
    updateInfo: UpdateShippingAddressInput,
  ): Promise<ListAddress | undefined> {
    try {
      const shippingAddress = await this.shippingAddressModel.findOneAndUpdate(
        {
          user: Types.ObjectId(userId),
          _id: Types.ObjectId(addressId),
        },
