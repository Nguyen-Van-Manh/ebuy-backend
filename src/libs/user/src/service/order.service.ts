...
export class OrderService {
  ...
  async adjustItem(
    ...
    try {
      const variant = await this.variantService.getVariant(
        Types.ObjectId(variantId),
      );
      if (!variant) {
      ...
      const orderLine = await this.orderLineModel.findById(
        Types.ObjectId(orderLineId),
      );
      if (!order) {
      ...
      throw new InternalServerErrorException(
        e.message || 'AN ERROR OCCURRED IN PROCESS',
      );
    }
  }
  async addItemToOrder(
    ...
    try {
      if (quantity <= 0) {
        throw new HttpException(
          'Order quantity must be greater 0',
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  async orderLinesByOrderId(orderId: Types.ObjectId) {
    const lines = await this.orderLineModel
      .find({
        orderId: orderId,
      })
  ...
  async setShippingAddressForOrder(
    ...
    try {
      const checkAddress = await this.shippingAddressService.getShippingAddressDetail(
        userId,
        Types.ObjectId(addressId),
      );
      if (!checkAddress) {
      ...
      throw new InternalServerErrorException(
        e?.message || 'An error occurred while processing request',
      );
    }
  }
