...
import {
  ProductPromotion,
  ProductPromotionDocument,
} from '../schema/product-promotion.schema';
...
export class ProductPromotionService {
  constructor(
    @InjectModel(ProductPromotion.name)
    private productPromotionModel: Model<ProductPromotionDocument>,
    private productService: ProductService,
  ) {}

  async addPromotionForProduct(
    productId: string,
    ...
    try {
      ...
      throw new HttpException(
        'ERROR: There is only one promotion per product',
  ...
  async promotionDetail(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        e?.message || 'An error occurred while processing requesr',
      );
    }
  }
  ...
  async update(
    ...
    try {
      ...
      if (!promotion) {
        throw new HttpException(
          'Promotion not exist on this product',
          HttpStatus.BAD_REQUEST,
        );
      }
